...
public class UserRedisRepository {
    ...
    public void saveVideoDetailsToRedis(Video video,long timeoutInSeconds){
        ...
        stringRedisTemplate.opsForHash().put(redisKey,"videoHeat",String.valueOf(video.getVideoHeat()));
        stringRedisTemplate.opsForHash().put(redisKey,"videoFutureViews",String.valueOf(video.getVideoFutureViews()));
        stringRedisTemplate.opsForHash().put(redisKey,"videoTitle",video.getVideoTitle());
        //设置过期时间
        stringRedisTemplate.expire(redisKey, Duration.ofSeconds(timeoutInSeconds));
